...
async function loadProfiles() {
  ...
  state.profiles.forEach((p) => {
    ...
    tr.innerHTML = `<td>${p.id_profile}</td><td>${p.profile}</td><td><button class="btn" onclick="editProfile(${p.id_profile}, '${esc(p.profile)}')">Editar</button><button class="btn btn-danger" onclick="deleteProfile(${p.id_profile})">Eliminar</button></td>`;
...
async function loadObjects() {
  ...
  state.objects.forEach((obj) => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${obj.id_object}</td><td>${obj.object}</td><td><button class="btn" onclick="editObject(${obj.id_object}, '${esc(obj.object)}')">Editar</button><button class="btn btn-danger" onclick="deleteObject(${obj.id_object})">Eliminar</button></td>`;
    tbody.appendChild(tr);
  });
}
...
function renderMethodsTable() {
  ...
  filteredMethods.forEach((m) => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${m.id_method}</td><td>${m.method}</td><td>${m.object}</td><td><button class="btn" onclick="editMethod(${m.id_method}, '${esc(m.method)}', ${m.fk_id_object})">Editar</button><button class="btn btn-danger" onclick="deleteMethod(${m.id_method})">Eliminar</button></td>`;
    tbody.appendChild(tr);
  });
}
...
async function loadUsers() {
  ...
  state.users.forEach((u) => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${u.id_user}</td><td>${u.name} ${u.last_name}</td><td>${u.email}</td><td>${u.number_id}</td><td>${u.profile}</td><td><button class="btn" onclick="editUser(${u.id_user}, ${u.id_person}, '${esc(u.name)}', '${esc(u.last_name)}', '${u.birth_date || ''}', '${esc(u.email)}', '${esc(u.number_id)}', ${u.id_profile})">Editar</button><button class="btn btn-danger" onclick="deleteUser(${u.id_user})">Eliminar</button></td>`;
    tbody.appendChild(tr);
  });
}
...
function renderPermissionsGrid(visibleMethodsParam, activeMethodIdsParam) {
  ...
  filteredMethods.forEach((m) => {
    ...
    card.className = `perm-card ${isActive ? 'active' : ''}`;
    ...
    content.innerHTML = `<label class="font-bold">${m.object}</label><p class="text-xs">${m.method}</p>`;
...
async function savePermissions() {
  ...
  const selectedMethods = Array.from(document.querySelectorAll('#methodsGrid .custom-checkbox'))
